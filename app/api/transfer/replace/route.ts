...
export async function POST(request: Request) {
  ...
  try {
    ...
    const clientEmail = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_EMAIL
    const privateKey = process.env.NEXT_PUBLIC_GOOGLE_PRIVATE_KEY
    const requirementsFolder =
      process.env.NEXT_PUBLIC_GOOGLE_DRIVE_REQUIREMENT_FOLDER
    const entryRequirementsFolder =
      process.env.NEXT_PUBLIC_GOOGLE_DRIVE_ENTRY_REQUIREMENT_FOLDER
    ...
    try {
      ...
      const newFileInEntryRequirements =

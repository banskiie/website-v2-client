  ...
  const form = useForm({
    ...
    listeners: {
      onChange: ({ formApi, fieldApi }) => {
        ...
        if (fieldApi.name.includes('connectedPlayer') || fieldApi.name.includes('isPlayer')) {
          setValidationErrors({});
        }
  ...
  const handleFormSubmit = async () => {
    ...
    if (needsPlayer1Selection) {
      ...
      if (!player1Config.isNew && !player1Config.connected) {
        ...
        setValidationErrors(prev => ({ ...prev, player1: "Player 1 is not assigned" }));
        return;
      }

      if (!player1Config.isNew && player1Config.connected && player1Config.hasDocuments && !player1Config.hasSelections) {
      ...
      if (entry?.player2Entry) {
        if (!player2Config.isNew && !player2Config.connected) {
          ...
          setValidationErrors(prev => ({ ...prev, player2: "Player 2 is not assigned" }));
          return;
        }

        if (!player2Config.isNew && player2Config.connected && player2Config.hasDocuments && !player2Config.hasSelections) {
            ...
            <FieldSet className="flex flex-col gap-2.5 h-[75vh] overflow-y-auto pr-2">
              <form.Field
                ...
                children={(field) => {
                  ...
                  const hasError = validationErrors[tab === 'player1' ? 'player1' : 'player2'];
                    ...
                    <Field data-invalid={isInvalid || hasError}>
                      <div className="flex items-center py-1">
                        <Checkbox
                          ...
                          className="mr-2"
                          aria-invalid={Boolean(isInvalid || hasError) ? "true" : "false"}
                          disabled={isLoading}
                        />
                        <div className="grid">
                          <FieldLabel htmlFor={field.name} className={hasError ? "text-destructive" : ""}>
                              ...
                              >
                                ...
                                <CommandList className="max-h-72 overflow-y-auto">
                                  <CommandGroup>
                                    ...
                                    {players?.map(
                                      (o: { value: string; label: string; hasEarlyBird: boolean }) => (
                                        <CommandItem
                                          key={o.value}
                                          value={o.value}
                                          onSelect={(v) => {

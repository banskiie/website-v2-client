...
import { ITournamentInput, ITournamentNode } from "@/types/tournament.interface"
...
const TOURNAMENT_CHANGED = gql`
  subscription TournamentChanged {
    tournamentChanged {
      type
      tournament {
        _id
...
const ActionsColumn = ({ data }: { data?: ITournamentInput }) => {
  const tournament = useMemo(() => data, [data])
...
const Page = () => {
  ...
  // Subscription to Tournament Changes
  useEffect(() => {
    const unsubscribe = subscribeToMore({
      document: TOURNAMENT_CHANGED,
      updateQuery: (prev: any, { subscriptionData }: any) => {
        if (!subscriptionData.data) return prev
        const { type, tournament, tournaments } =
          subscriptionData.data.tournamentChanged
        // Update the tournaments list based on the type of change
        switch (type) {
          case "CREATE":
            // Add the new tournament to the top of the list
            const newTournament = tournament
            const newTournamentExists = prev.tournaments.edges.find(
              (edge: any) => edge.node._id === newTournament?._id
            )
            if (newTournamentExists || search || sort || filter.length > 0)
              return prev // Skip updating during search/sort/filter
            toast.success(
              `Tournament (${newTournament?.name}) has been created.`
            )
            return Object.assign({}, prev, {
              tournaments: {
                ...
                edges: [
                  { cursor: newTournament?._id, node: newTournament },
                  ...prev.tournaments.edges,
                ],
              },
            })
          case "UPDATE":
            // Update the existing tournament in the list
            const updatedTournament = tournament
            if (search || sort || filter.length > 0) return prev // Skip updating during search/sort/filter
            toast.success(
              `Tournament (${updatedTournament?.name}) has been updated.`
            )
            return Object.assign({}, prev, {
              tournaments: {
                ...prev.tournaments,
                edges: prev.tournaments.edges.map((edge: any) =>
                  edge.node._id === updatedTournament._id
                    ? { ...edge, node: updatedTournament }
                    : edge
                ),
              },
            })
          case "DELETE":
            ...
            if (search || sort || filter.length > 0) return prev // Skip updating during search/sort/filter
            toast.success(
              `Tournament (${deletedTournament?.name}) has been deleted.`
            )
            return Object.assign({}, prev, {
              tournaments: {
                ...prev.tournaments,
                total: prev.tournaments.total - 1,
                edges: prev.tournaments.edges.filter(
                  (edge: any) => edge.node._id !== deletedTournament._id
                ),
              },
            })
          case "BATCH_UPDATE":
            ...
            if (search || sort || filter.length > 0) return prev // Skip updating during search/sort/filter
            toast.success(
              `Batch update successful for ${updatedTournaments.length} tournaments.`
            )
            const updatedIds = new Set(
              updatedTournaments.map((u: any) => u._id)
            )
            return Object.assign({}, prev, {
              tournaments: {
                ...prev.tournaments,
  ...
  const { total, nodes, pageInfo } = useMemo(() => {
    const nodes = data?.tournaments.edges.map((edge: any) => edge.node) || []
    const pageInfo = data?.tournaments.pageInfo

    setPage((prev) => ({
      ...prev,
      max: data?.tournaments.pages || 1,
    }))
  ...
  // Table Columns
  const columns: ColumnDef<ITournamentNode>[] = useMemo(
    () => [
      {
        id: "select",
        footer: () => {
          return (
            <Checkbox
              checked={
                selectedIds.size === data?.tournaments.edges.length &&
                data?.tournaments.edges.length > 0
              }
              className="hover:cursor-pointer"
              onCheckedChange={(value: boolean) => {
                if (value) {
                  const allIds = new Set<string>(
                    data?.tournaments.edges.map((edge: any) => edge.node._id)
                  )
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const startDate = new Date(row.original.tournamentStart)
          const endDate = new Date(row.original.tournamentEnd)
          return (
            <span>
              {formatDateRange(startDate, endDate)} (
              <span>{formatDistanceStrict(startDate, endDate)})</span>
            </span>
          )
        },
        size: 240,
      ...
      {
        accessorKey: "hasEarlyBird",
        header: () => (
          <SortHeader
            label="Early Bird"
            sortKey="hasEarlyBird"
            sortState={sort}
            onSortChange={onSort}
          />
        ),
        footer: () => (
          <ColumnFilter
            label="Early Bird"
            filterKey="hasEarlyBird"
            filterType="BOOLEAN"
            filterValue={filter}
            onFilterChange={onFilter}
          />
        ),
        cell: ({ row }) => <StatusBadge status={row.original.hasEarlyBird} />,
      },
      {
        accessorKey: "hasFreeJersey",
        header: () => (
          <SortHeader
            label="Free Jersey"
            sortKey="hasFreeJersey"
            sortState={sort}
            onSortChange={onSort}
          />
        ),
        footer: () => (
          <ColumnFilter
            label="Free Jersey"
            filterKey="hasFreeJersey"
            filterType="BOOLEAN"
            filterValue={filter}
            onFilterChange={onFilter}
          />
        ),
        cell: ({ row }) => <StatusBadge status={row.original.hasFreeJersey} />,
      },
      {
    ...
    [sort, onSort, filter, onFilter, selectedIds, data?.tournaments]
  )
      ...
      await fetchMore({
        ...
        updateQuery: (prev: any, { fetchMoreResult: more }: any) => {
          ...
          return {
            tournaments: {
              ...prev.tournaments,
              edges: [...prev.tournaments.edges, ...more.tournaments.edges],
              pageInfo: more.tournaments.pageInfo,
            },
          }
        },
      })
  ...
  return (
    <div className="w-full h-full flex-1 flex flex-col gap-2">
      <div className="flex items-center gap-1 text-slate-900 -mb-1.5">
        ...
        <Label className="text-2xl">Tournaments</Label>
      </div>

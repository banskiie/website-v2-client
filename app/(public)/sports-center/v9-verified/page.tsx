...
function VerifiedEntriesContent() {
  ...
  useEffect(() => {
    if (data?.verifiedEntriesByTournament) {
      const allEventIds = data.verifiedEntriesByTournament.map(
        (event) => event.eventId,
      );
  ...
  const uniqueEventNames = data?.verifiedEntriesByTournament
    ? Array.from(
        new Set(
          data.verifiedEntriesByTournament.map((event) => event.eventName),
        ),
      ).sort()
    : [];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-[#eef3ff] to-[#e2e8ff] relative overflow-hidden">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 mt-5">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">
                {tournamentName} Verified Entries
              </h1>
              <p className="text-gray-600">
                View all verified tournament entries for {tournamentName}
              </p>
              {/* <div className="mt-2">
                    ...
                    >
                      <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gray-50">
                        <div className="flex items-center gap-4 text-left">
                          <div
                            className={`p-2 rounded-lg ${event.totalEntries > 0 ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"}`}
                          >
                            <Box size={20} />
                          </div>
                          <div>
                            <h3 className="font-medium text-gray-900 text-md underline underline-offset-2">
                              {event.eventName}
                            </h3>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-gray-500">
                                ({event.totalEntries}{" "}
                                {event.totalEntries === 1 ? "entry" : "entries"}
                                )
                              </span>
                            </div>
                          </div>
                              ...
                              {event.entries.map((entry, index) => (
                                ...
                                >
                                  <div className="flex items-start justify-between flex-col">
                                    <div className="flex items-baseline gap-2 underline-offset-2">
                                      <span className="text-lg font-bold text-gray-900">
                                        {entry.entryNumber}
                                      </span>
                                    </div>
                                    {(tournamentData as any)?.tournament
                                      .settings.showVerified && (
                                      <>
                                        <div>
        ...
        {!loading &&
          !data?.verifiedEntriesByTournament?.length &&
          !searchQuery &&
          selectedEventFilter === "all" && (
            <Card>
              <CardContent className="py-12">

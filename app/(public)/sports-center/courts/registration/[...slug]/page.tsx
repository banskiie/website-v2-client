        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6 text-center transform transition-all duration-300 scale-100 border border-green-200">
            ...
            <div className="mt-4 p-4 bg-linear-to-r from-green-50 to-teal-50 rounded-xl border border-green-300">
              <div className="flex justify-between items-center">
                <div className="text-left">
                  <div className="font-semibold text-green-800 text-[15px] ">
                    {isDoubles ? "Doubles" : "Single"} Category Total Amount
                  </div>
          ...
          <div className="p-4 bg-green-50 text-green-800 rounded-xl border border-green-200 text-left mb-4">
            ...
            <p className="text-green-700 text-xs">
              After submitting this registration, please wait for an approval
              notice via{" "}
              <strong className="underline underline-offset-2">email</strong>{" "}
              before making any payment. The{" "}
              <strong className="underline underline-offset-2">
                payment instructions
              </strong>{" "}
              will also be sent to your email. I advise{" "}
              <strong className="underline underline-offset-2">
                not to pay
              </strong>{" "}
              until you receive the approval notice. Please complete your
              payment within the payment deadline.
            </p>
          </div>
  ...
  >
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-xl shadow-2xl border border-green-200 max-w-md mx-4">
      ...
      <p className="text-gray-600 text-center mb-4">
        Please wait while we submit your registration. This may take a few
        moments.
      </p>
    ...
    if (validationErrors && Array.isArray(validationErrors)) {
      ...
      validationErrors.forEach((err: any) => {
        ...
        if (frontendFieldName) {
          ...
          if (
            backendPath.includes("birthDate") ||
            backendPath.includes("birthdate") ||
      ...
      try {
            ...
            toast.error(
              <div>
                ...
                <p className="text-sm mt-1">
                  Found {uniqueAllErrors.length} validation error(s). Please
        ...
        const createPlayerEntry = (
          ...
          const baseEntry = {
            ...
            birthDate: convertToISODateTime(
              playerData[`player${playerNum}Birthday`],
            ),
        ...
        const input = {
          ...
          ...(event?.type === "DOUBLES" && {
            player2Entry: createPlayerEntry(
              finalFormData,
              2,
              documentUrlPlayer2,
            ),
  ...
  const DocumentTypeSelector = ({ playerNum }: { playerNum: number }) => {
    ...
    return (
      <Field className="text-left">
        <FieldLabel
          htmlFor={`documentType${playerNum}`}
          className="text-green-800"
        >
          Document Type (Player {playerNum}){" "}
    ...
    <div className="bg-linear-to-br from-green-50 to-emerald-100">
      ...
      <AnimatePresence>
        ...
        {(isSubmitting || submitting) && (
          <FormSubmittingOverlay key="submitting-overlay" />
        )}
      </AnimatePresence>
            ...
            <div className="max-w-5xl mx-auto p-4 sm:p-6 bg-white rounded-xl shadow-md border border-green-200">
              <div className="text-center space-y-4 mt-4">
                <CardTitle className="flex items-center gap-2 text-green-800 justify-center">
                  ...
                  <span className="text-md md:text-md lg:text-lg xl:text-lg 2xl:text-lg font-semibold">
                    Club Information
                  </span>
                </CardTitle>

                <p className="text-green-800 text-xs">
                  Provide your club details. You can use these contact details
                  for players below.
                </p>
                ...
                <FieldGroup className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
                  {(["clubEmail", "clubContactNumber"] as FormFieldNames[]).map(
                    (name) => (
                      <form.Field
                        key={name}
                        name={name}
                ...
                <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200 max-w-2xl mx-auto">
                  <p className="text-green-700 text-sm text-left">
                    ...
                    <span className="font-medium">Button</span> in{" "}
                    <span className="font-bold">
                      PLAYER CONTACT INFORMATION
                    </span>{" "}
                    below to automatically fill in their contact information
                    with these club details that you input.
                  </p>
                </div>
              </div>
            </div>
...
{/* Dari Address */}
                              ...
                              >
                                <div className="border-2 border-dashed border-green-300 rounded-2xl p-4 sm:p-6 bg-white flex flex-col items-center justify-center text-center gap-4">
                                  <div className="w-full flex flex-col text-left">
                                    <FieldLabel
                                      htmlFor={field.name}
                                      className="text-green-800 font-bold text-md text-center md:text-lg lg:text-lg xl:text-lg 2xl:text-lg"
                                    >
                                      Upload your Document for Verification{" "}
                                      <span className="text-red-500">*</span>
                                    </FieldLabel>
                                    <p className="text-gray-600 text-xs md:text-sm lg:text-sm xl:text-sm 2xl:text-sm mt-1">
                                      To complete your registration, we need a
                                      clear copy of your{" "}
                                      {selectedDocumentType
                                        .replaceAll("_", " ")
                                        .toLowerCase()}
                                      . Make sure the details are clearly
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <Button
                                            type="button"
                                            onClick={() =>
                                              handleRemoveFile(playerNum, field)
                                            }
                                            disabled={isUploading || submitting}
                                    ...
                                    {preview &&
                                      ...
                                      <Image
                                        ...
                                        onClick={() =>
                                          ...
                                          handlePreviewFile(
                                            file,
                                            selectedDocumentType.replaceAll(
                                              "_",
                                              " ",
                                            ),
                                          )
                                  ...
                                  >
                                    {isUploading &&
                                      ...
                                      <>
                                        ...
                                        <span className="text-xs text-gray-500 mt-1">
                                          Supports images (JPEG, PNG, JPG, WEBP)
                                          and PDF files up to 10MB
                                        </span>
                                      </>
                                    )}
                                    <input
                                      ...
                                      className="hidden"
                                      onChange={(e) =>
                                        handleFileUpload(playerNum, e, field)
                                      }
                                      disabled={isUploading || submitting}
                                    />
                                  </label>
                      ...
                      <div className="flex items-start gap-4 p-4 bg-green-100 rounded-lg border-2 border-green-300 mb-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-green-700">
                            Click here to Turn{" "}
                            <span className="font-semibold">ON</span> to{" "}
                            <span className="italic font-medium underline underline-offset-2">
                              copy the club email & contact number
                            </span>{" "}
                            from the{" "}
                      ...
                      <FieldGroup className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                        {(
                          [
                            `player${playerNum}Email`,
                            `player${playerNum}ContactNumber`,
                          ] as FormFieldNames[]
                        ).map((name) => (
                          <form.Field
                            key={name}
